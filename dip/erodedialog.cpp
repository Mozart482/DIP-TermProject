...
ErodeDialog::ErodeDialog(QImage inputImage)
{
    srcImage = inputImage;
...
void ErodeDialog::setImage(QImage image, QLabel *label)
{
    QPixmap pix;
    pix.fromImage(image);
    label->setPixmap(pix);
...
void ErodeDialog::Erosion(QImage &src_image, QImage &dst_image)
{
    dst_image = src_image;
    int kernel[5][5] = {
        { 0,0,1,0,0 },
